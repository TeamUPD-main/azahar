...
static std::shared_ptr<IOSMotionFactory> motion;
...
std::unique_ptr<Input::ButtonDevice> ButtonFactory::Create(const Common::ParamPackage& params) {
    ...
    const int button_id = params.Get("code", 0);
...
void ButtonFactory::ReleaseAllKeys() {
    button_list->ChangeAllButtonStatus(false);
    analog_button_list->ChangeAxisValue(0, 0.0f);
    analog_button_list->ChangeAxisValue(1, 0.0f);
    analog_button_list->ChangeAxisValue(2, 0.0f);
    analog_button_list->ChangeAxisValue(3, 0.0f);
}

std::unique_ptr<Input::AnalogDevice> AnalogFactory::Create(const Common::ParamPackage& params) {
    const int analog_id = params.Get("code", 0);
...
void Init() {
    ...
    motion = std::make_shared<IOSMotionFactory>();
