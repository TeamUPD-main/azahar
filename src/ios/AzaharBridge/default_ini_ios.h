...
// All of these setting keys are either not currently used by iOS or are too
// niche to bother documenting, or some other explained reason.
constexpr std::array ios_config_omitted_keys = {
    ...
    Settings::Keys::last_artic_base_addr, // Stored as a Swift @AppStorage preference
    Settings::Keys::break_on_unmapped_memory_access, // Does nothing as the error is ignored
    Settings::Keys::use_gdbstub, // GDB functionality disabled by default on iOS
...
static const char* ios_config_default_file_content = (BOOST_HANA_STRING(R"(
[Controls]
# for motion input, the following devices are available:
#  - "motion_emu" (default) for emulating motion input from the device sensors. Required parameters:
#      - "update_period": update period in milliseconds (default to 100)
#      - "sensitivity": the coefficient converting sensor data to tilting angle (default to 0.01)
...
#  - "emu_window" (default) for emulating touch input from touch events to the emulation window. No parameters required
...
# if you have screen tearing, which is unusual on iOS.
# 0 (default): Off, 1: On
...
#        to select an image.
# ios (Default): uses the device camera via AVFoundation. You can specify the camera ID to use in the config field.
...
# Whether to enable PICA200 debugging (does nothing on iOS)
# 0 (default): Off, 1: On
