...
#include "ios/AzaharBridge/default_ini_ios.h"
...
Config::Config() {
    ...
    ios_config_loc = FileUtil::GetUserPath(FileUtil::UserPath::ConfigDir) + "config.ini";
    std::string ini_buffer;
    FileUtil::ReadFileToString(true, ios_config_loc, ini_buffer);
    if (!ini_buffer.empty()) {
        ios_config = std::make_unique<INIReader>(ini_buffer.c_str(), ini_buffer.size());
    }
...
bool Config::LoadINI(const std::string& default_contents, bool retry) {
    const std::string& location = this->ios_config_loc;
    if (ios_config == nullptr || ios_config->ParseError() < 0) {
        if (retry) {
...
void Config::ReadSetting(const std::string& group, Settings::Setting<bool>& setting) {
    setting = ios_config->GetBoolean(group, setting.GetLabel(), setting.GetDefault());
}

template <typename Type, bool ranged>
void Config::ReadSetting(const std::string& group, Settings::Setting<Type, ranged>& setting) {
    if constexpr (std::is_floating_point_v<Type>) {
        setting = ios_config->GetReal(group, setting.GetLabel(), setting.GetDefault());
    } else {
        setting = static_cast<Type>(ios_config->GetInteger(
...
void Config::ReadValues() {
    ...
    for (int i = 0; i < Settings::NativeButton::NumButtons; ++i) {
        ...
        Settings::values.current_input_profile.buttons[i] = ios_config->GetString(
    ...
    for (int i = 0; i < Settings::NativeAnalog::NumAnalogs; ++i) {
        ...
        Settings::values.current_input_profile.analogs[i] = ios_config->GetString(
    ...
    Settings::values.current_input_profile.motion_device = ios_config->GetString(
    ...
    if (ios_config->GetBoolean("Renderer", "use_frame_limit", true)) {
    ...
    int layoutInt = (int)ios_config->GetInteger(
